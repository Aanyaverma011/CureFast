...
//this function will remove dup. slot and return the slot filter by time
...
//this func accept the obj or json string
...
return res.status(500).json({
    ...
    message: "Server Misconfigured"
     ...
     const token = jwt.sign({id: doc._id.toString(), email: doc.email, role: "doctor"
    }, secret, { expiresIn: "7d" });
...
export const getDoctors = async (req, res) => {
  try {
    ...
    const total = await Doctor.countDocuments(match); //total number of doctors
...
//to get doctor by id ie. to fetch one doc
...
if(doc.password !== password) return req.status(401).json({
success: false,
...
delete out.password;
return res.json({ sucess: true, token, data: out });
