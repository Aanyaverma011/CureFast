...
import { BadgeIndianRupee, Calendar, CheckCircle, ClipboardList, Search, SortDesc, XCircle } from "lucide-react";
...
function normalizeService(doc) {
  ...
  doc.image ||
  doc.avatar ||
  (doc.image === null ? `https://i.pravatar.cc/150?u=${id}` : doc.image);
 ...
 <div className={serviceDashboardStyles.table.tabletView}>
    ...
    {filteredServices.length>INITIAL_COUNT && (
      <div className={serviceDashboardStyles.showMore.container}>
        ...
        {showAll ? "Show less" : `Show more (${serviceDashboardStyles.length - INITIAL_COUNT})`}
        </button>
        </div>
    )}
     </div>
